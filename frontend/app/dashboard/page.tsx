// File: frontend/app/dashboard/page.tsx (Final Version)
...
interface Order {
  ...
  total_price: number;
  created_at: string; // Comes as a string from JSON
  items: OrderItem[];
}
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        // Fetch page and order data in parallel for better performance
        const apiUrl = process.env.NEXT_PUBLIC_API_URL;
        const [pageRes, ordersRes] = await Promise.all([
          axios.get(`${apiUrl}/pages/me`, {
            headers: { Authorization: `Bearer ${token}` },
          }),
          axios.get(`${apiUrl}/orders/my-orders`, {
            headers: { Authorization: `Bearer ${token}` },
          })
        ]);
  ...
  // ... All handler functions (handlePageCreated, handleProductAdded, etc.) remain the same ...
  ...
  const apiUrl = process.env.NEXT_PUBLIC_API_URL;
  const confirmDelete = async () => { if (!productToDelete) return; try { await axios.delete(`${apiUrl}/products/${productToDelete.id}`, { headers: { Authorization: `Bearer ${token}` } }); handleProductDeleted(productToDelete); setIsDeleteModalOpen(false); setProductToDelete(null); } catch (err) { console.error("Failed to delete product", err); } };
      ...
      <div className="min-h-screen bg-gray-100">
        ...
        <main className="p-8">
          <div className="mx-auto max-w-3xl space-y-8">
            {/* ... Page Details & Products Cards are the same ... */}
            <div className="rounded-lg bg-white p-6 shadow-lg"> <div className="flex items-center justify-between"> <h2 className="text-3xl font-bold text-gray-800">{pageData.title}</h2> <button onClick={() => setIsEditPageModalOpen(true)} className="rounded-md bg-indigo-500 px-4 py-2 text-white hover:bg-indigo-600">Edit Page</button> </div> <p className="mt-2 text-gray-600">{pageData.description || 'No description provided.'}</p> </div>
            <div className="rounded-lg bg-white p-6 shadow-lg"> <div className="flex items-center justify-between"> <h3 className="text-2xl font-bold text-gray-800">Your Products/Services</h3> <button onClick={() => setIsAddProductModalOpen(true)} className="rounded-md bg-green-500 px-4 py-2 text-white hover:bg-green-600">Add New</button> </div> <ul className="mt-4 space-y-3"> {pageData.products.length > 0 ? (pageData.products.map((product) => (<li key={product.id} className="flex items-center justify-between rounded-md border p-3"><div><p className="font-semibold">{product.name}</p><p className="text-sm text-gray-500">₹{product.price.toFixed(2)}</p></div><div className="space-x-2"><button onClick={() => openEditModal(product)} className="text-sm font-medium text-indigo-600 hover:text-indigo-800">Edit</button><button onClick={() => openDeleteModal(product)} className="text-sm font-medium text-red-600 hover:text-red-800">Delete</button></div></li>))) : (<p className="text-center text-gray-500">You haven't added any products yet.</p>)} </ul> </div>

            {/* --- NEW RECEIVED ORDERS CARD --- */}
                      ...
                      <ul className="mt-2 text-sm text-gray-600">
                        {order.items.map((item, index) => (
                          <li key={index} className="flex justify-between">
                            ...
                            <span>(₹{item.price_per_item.toFixed(2)} each)</span>
                          </li>
                        ))}
                      </ul>
                    </div>
