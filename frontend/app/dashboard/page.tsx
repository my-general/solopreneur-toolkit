// File: frontend/app/dashboard/page.tsx (Corrected for Production)
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        // 2. Use the API_URL variable for all fetch calls.
        const [pageRes, ordersRes] = await Promise.all([
          axios.get(`${API_URL}/pages/me`, { headers: { Authorization: `Bearer ${token}` } }),
  ...
  const confirmDelete = async () => { 
    ...
    try { 
      // 3. Use the API_URL variable for the delete call.
      await axios.delete(`${API_URL}/products/${productToDelete.id}`, { headers: { Authorization: `Bearer ${token}` } }); 
      handleProductDeleted(productToDelete); 
